...
async function start(): Promise<void> {
  console.log('🚀 SentinelGlobal Worker v2 — Real Time Intelligence');
  ...
  // 1. Event pipeline: Collector → Risk Engine → AI → Notifications
  ...
  // 4. Periodic broadcast loop (risk score + AI)
  setInterval(async () => {
    const now = Date.now();
